...
__global__ void sgemm(float * __restrict__ A, float * __restrict__ B, float * __restrict__ C, int n, float a, float b) {
  ...
  for(int i=0; i<4; i++) {
    if (global_a_r[i] < n && global_a_c[i] < n) {
       float4 tmp = reinterpret_cast<const float4*>(src_A_ptr[i])[0];
       As[load_a_row[i]][load_a_col[i]]   = tmp.x;
       As[load_a_row[i]][load_a_col[i]+1] = tmp.y;
       As[load_a_row[i]][load_a_col[i]+2] = tmp.z;
       As[load_a_row[i]][load_a_col[i]+3] = tmp.w;
    } else {
       As[load_a_row[i]][load_a_col[i]]   = 0.0;
       As[load_a_row[i]][load_a_col[i]+1] = 0.0;
       As[load_a_row[i]][load_a_col[i]+2] = 0.0;
       As[load_a_row[i]][load_a_col[i]+3] = 0.0;
    }

    if (global_b_r[i] < n && global_b_c[i] < n) {
       float4 tmp = reinterpret_cast<const float4*>(src_B_ptr[i])[0];
       Bs[load_b_row[i]][load_b_col[i]]   = tmp.x;
       Bs[load_b_row[i]][load_b_col[i]+1] = tmp.y;
       Bs[load_b_row[i]][load_b_col[i]+2] = tmp.z;
       Bs[load_b_row[i]][load_b_col[i]+3] = tmp.w;
    } else {
       Bs[load_b_row[i]][load_b_col[i]]   = 0.0;
       Bs[load_b_row[i]][load_b_col[i]+1] = 0.0;
       Bs[load_b_row[i]][load_b_col[i]+2] = 0.0;
       Bs[load_b_row[i]][load_b_col[i]+3] = 0.0;
    }
  ...
  for (int k = 0; k < TILE_WIDTH; ++k) {
    float a_vals[4];
    float b_vals[4];
    for(int i=0; i<4; i++) { a_vals[i] = As[ty + i*16][k]; }
    for(int j=0; j<4; j++) { b_vals[j] = Bs[k][tx + j*16]; }
    for(int i=0; i<4; i++) {
      for(int j=0; j<4; j++) {
