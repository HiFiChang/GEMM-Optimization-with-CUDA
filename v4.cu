...
__global__ void sgemm(float *A, float *B, float *C, int n, float a, float b) {
  ...
  for(int i=0; i<4; i++) {
    ...
    int row_vec = load_idx / 16;
    int col_vec = load_idx % 16;

    int row_local = row_vec;
    int col_local = col_vec * 4;

    int row_global_A = by * TILE_WIDTH + row_local;
    int col_global_A = m * TILE_WIDTH + col_local;

    if (row_global_A < n && col_global_A < n) {
       float4 tmp = reinterpret_cast<float4*>(&A[row_global_A * n + col_global_A])[0];
       As[row_local][col_local] = tmp.x;
       As[row_local][col_local+1] = tmp.y;
       As[row_local][col_local+2] = tmp.z;
       As[row_local][col_local+3] = tmp.w;
    } else {
       As[row_local][col_local] = 0.0;
       As[row_local][col_local+1] = 0.0;
       As[row_local][col_local+2] = 0.0;
       As[row_local][col_local+3] = 0.0;
    }

    int row_global_B = m * TILE_WIDTH + row_local;
    int col_global_B = bx * TILE_WIDTH + col_local;

    if (row_global_B < n && col_global_B < n) {
       float4 tmp = reinterpret_cast<float4*>(&B[row_global_B * n + col_global_B])[0];
       Bs[row_local][col_local] = tmp.x;
       Bs[row_local][col_local+1] = tmp.y;
       Bs[row_local][col_local+2] = tmp.z;
       Bs[row_local][col_local+3] = tmp.w;
    } else {
       Bs[row_local][col_local] = 0.0;
       Bs[row_local][col_local+1] = 0.0;
       Bs[row_local][col_local+2] = 0.0;
       Bs[row_local][col_local+3] = 0.0;
    }
  }
  ...
  for (int k = 0; k < TILE_WIDTH; ++k) {
    ...
    for(int i=0; i<4; i++) {
      a_vals[i] = As[ty + i*16][k];
    }

    for(int j=0; j<4; j++) {
      b_vals[j] = Bs[k][tx + j*16];
    }

    for(int i=0; i<4; i++) {
      for(int j=0; j<4; j++) {
        c[i][j] += a_vals[i] * b_vals[j];
      }
    }
  }
